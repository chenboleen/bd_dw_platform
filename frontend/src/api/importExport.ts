...
export const createExportTask = (data: ExportRequest) =>
  apiClient.post<ExportStatusResponse>('/export', data)

/** 获取导出状态 */
export const getExportStatus = (taskId: number) =>
  apiClient.get<ExportStatusResponse>(`/export/${taskId}/status`)

/** 下载导出文件 */
export const downloadExportFile = (taskId: number) =>
  apiClient.get(`/export/${taskId}/download`, { responseType: 'blob' })

/** 获取导出任务列表 */
export const listExportTasks = (page = 1, pageSize = 20) =>
  apiClient.get('/export/tasks', { params: { page, pageSize } })
...
export const importFromJson = (file: File) => {
  const formData = new FormData()
  formData.append('file', file)
  return apiClient.post<ImportResult>('/import/json', formData, {
    headers: { 'Content-Type': 'multipart/form-data' }
  })
}
