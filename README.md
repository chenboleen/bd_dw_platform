...
- **表元数据管理**：创建、查询、更新、删除表和字段元数据；新建/编辑表时支持选择数据域（数据目录），存储路径根据数据库名和表名自动生成
- **血缘关系追踪**：可视化数据血缘 DAG 图谱，节点按角色着色（中心表橙色、上游表绿色、下游表蓝色），支持影响分析
- **全文搜索**：基于 Elasticsearch 的多字段搜索和自动补全
- **数据目录**：层级目录组织，最多支持 5 级分类；提供扁平列表接口供表单下拉选择
- **数据质量**：质量指标记录和趋势分析
- **变更历史**：完整的元数据变更审计追踪，记录实体名称和操作人信息
